...
It is symmetric with the source SDK ([`@ontrove/extend/source`](https://www.npmjs.com/package/@ontrove/extend/source))
...
```bash
npm install @ontrove/extend/toolkit zod    # or: bun add @ontrove/extend/toolkit zod
```
...
`import { z } from '@ontrove/extend/toolkit'` and never import `zod` directly. Modern npm/bun
install the peer automatically; the explicit `zod` above is just belt-and-suspenders.

## Quickstart

Your `server.ts` must `export default defineToolkit(...)`:
...
`defineToolkit(config, { fetchImpl })`.
