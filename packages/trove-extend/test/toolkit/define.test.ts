...
describe('defineToolkit — annotation defaulting', () => {
...
describe('defineToolkit — default fetch wiring', () => {
  ...
  it('uses globalThis.fetch when no fetchImpl is injected (ctx.fetch egress)', async () => {
    ...
    expect(new Headers(init.headers).get('user-agent')).toContain('@ontrove/extend/toolkit');
  });
});
 ...
 */
describe('defineToolkit — declared output schemas are enforced', () => {
