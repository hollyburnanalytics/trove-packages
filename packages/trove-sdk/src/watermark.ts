/**
 * Reading and writing the typed watermarks a source resumes from.
 *
 * A feed's cursor is stored as an opaque JSON string and handed back to the
 * next run as `ctx.cursor`. {@link Watermark} says what that string may
 * contain; this module is the code that produces it, so the shape and the
 * bytes now live together.
 *
 * That pairing is the point. `test/watermark-contract.test.ts` names three
...
/**
 * Default cap on how many entries an `idSet` watermark retains.
 *
...
/**
 * Read a `date` watermark as a `Date`.
 *
 ...
 * @returns The watermark date, or `undefined` when there is not a usable one.
 */
export function readDateWatermark(cursor?: unknown): Date | undefined {
...
/**
 * Build a typed `date` watermark from an ISO-8601 string.
 *
 ...
 * @param args.inclusive - See {@link dateWatermark}.
 * @returns The cursor to return from `sync` — a new watermark when it is safe
 *   to advance, otherwise exactly what was passed in.
 */
export function advanceDateWatermark({
  previous,
  maxIso,
  anyFailed,
  inclusive = false,
...
/**
 * Read an `idSet` watermark as a string array.
 *
 * @param cursor - The source's previous cursor (`ctx.cursor`).
...
/**
 * Build a typed `idSet` watermark: deduped, bounded to `max` entries, and then
 ...
 * @returns The watermark to return from `sync`, carrying the numeric cap that
 ...
 */
export function idSetWatermark(
  values: readonly string[],
  max: number = DEFAULT_ID_SET_MAX,
): Extract<Watermark, { type: 'idSet' }> {
