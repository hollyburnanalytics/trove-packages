 ...
 * SourceContext} and a toolkit's `ToolContext`. Both `extend` it, so the
 * things an author uses constantly (a credential, a guarded fetch, a log line,
 * the clock) are learned once and are identical on both sides.
 *
 * They EXTEND it now. The two halves used to be separate packages that could
 * not import each other, so each declared this shape independently and a
 * type-level assertion in a third package held them together — it worked, but
 * it made agreement something to be checked rather than something structural.
 * One declaration needs no assertion.
 *
