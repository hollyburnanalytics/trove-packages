...
/**
 * The ISO value carried by a date watermark, or `''` when the cursor is not one.
 *
 * @param cursor - A cursor a writer returned.
 * @returns The `value` field, so a test can compare dates without re-narrowing.
 */
function dateCursorValue(cursor: Watermark | undefined): string {
...
describe('date watermark', () => {
  ...
  it('round-trips what the writer produced', () => {
    const written = dateWatermark('2024-01-10T00:00:00.000Z');
    const stored: unknown = JSON.parse(JSON.stringify(written));
    expect(readDateWatermark(stored)).toEqual(new Date('2024-01-10T00:00:00.000Z'));
  });
  ...
  it('returns undefined for a stored value that is not a date watermark at all', () => {
    ...
    expect(readDateWatermark('2024-01-10')).toBeUndefined();
    expect(readDateWatermark({ type: 'date', value: 1_704_844_800_000 })).toBeUndefined();
    expect(readDateWatermark({ type: 'idSet', values: ['a'] })).toBeUndefined();
  });
  ...
  it('writes `inclusive` only when it was asked for', () => {
    ...
    expect(dateWatermark('2024-01-10T00:00:00.000Z', { inclusive: false })).not.toHaveProperty(
      'inclusive',
    );
    expect(dateWatermark('2024-01-10T00:00:00.000Z', { inclusive: true })).toEqual({
      type: 'date',
      value: '2024-01-10T00:00:00.000Z',
      inclusive: true,
    });
...
describe('idSet watermark', () => {
  ...
  it('round-trips through storage back into the reader', () => {
    const written = idSetWatermark(['a', 'b']);
    const stored: unknown = JSON.parse(JSON.stringify(written));
...
describe('advanceDateWatermark', () => {
  const previous: Watermark = dateWatermark('2026-01-01T00:00:00.000Z');
  ...
  it('holds an absent cursor rather than inventing one', () => {
    expect(
      advanceDateWatermark({ previous: undefined, maxIso: undefined, anyFailed: false }),
    ).toBeUndefined();
  });
...
describe('the cursor byte budget', () => {
  ...
  it('leaves an ordinary blog untouched', () => {
    ...
    expect(idSetWatermark(urls).values).toEqual(urls);
  });
  ...
  it('measures bytes, not characters', () => {
    ...
    const cursor = idSetWatermark(ids);
  ...
  it('drops a single id that cannot fit, rather than emitting a refused cursor', () => {
    ...
    expect(idSetWatermark([monster]).values).toEqual([]);
  });
});
