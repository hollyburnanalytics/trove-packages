 ...
 *
 * This happened. `@ontrove/sdk@0.10.0` shipped a contract module whose JSON
 * import carried no `with { type: 'json' }` attribute. Node's ESM loader
 * refuses that outright, so the first consumer to `import '@ontrove/sdk/contract'`
...
describe('the built package', () => {
  ...
  it('imports its main entry under plain Node', () => {
    expect(inNode("import('@ontrove/sdk').then((m) => console.log(typeof m.defineSource));")).toBe(
      'function',
    );
  });
  ...
  it('imports the contract subpath under plain Node', () => {
    ...
    const out = inNode(
      "import('@ontrove/sdk/contract').then((m) => console.log(m.contract.version, m.CASE_SECTIONS.length));",
    );
    expect(out).toBe('1 5');
  });
  ...
  it('exposes the raw contract JSON as a file a non-TypeScript reader can open', () => {
    ...
    const out = inNode(
      "import('node:fs').then(async (fs) => {" +
        ...
        "const path = createRequire(process.cwd() + '/x.js').resolve('@ontrove/sdk/contract/source-invoke.json');" +
