/**
 * Public type surface for the `@ontrove/extend/toolkit` SDK.
 *
...
export interface ToolContext extends ExtensionContext {
   ...
   * here.
   *
   * The same shape a source gets from `ctx.config` in `@ontrove/extend/source`.
   */
...
/**
 * Declarative OAuth2 client-credentials auth. When set on {@link ToolkitConfig},
...
export type ToolCallResult = ToolCallOk | ToolCallErr;

/** Stable machine-readable error codes the SDK emits. */
export type ToolErrorCode = 'INVALID_PARAMS' | 'UNKNOWN_TOOL' | 'TOOL_ERROR' | 'BAD_REQUEST';

/**
 * The compiled server, produced by {@link defineToolkit}. It carries the
