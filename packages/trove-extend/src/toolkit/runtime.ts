 ...
 *
 * A deployed server's bundle re-exports its `defineToolkit(...)` value as
...
import type { ToolCall, ToolCallResult, ToolkitDefinition, ToolListEntry } from './types.js';
...
/**
 * Coerce an arbitrary parsed body into an {@link ToolCall}, filling defaults.
 *
 * @param body - The parsed request JSON.
 * @returns A normalized call, or `null` if it cannot be a tool call.
 */
function toCall(body: unknown): ToolCall | null {
 ...
 */
export function dispatch(server: ToolkitDefinition, call: ToolCall): Promise<ToolCallResult> {
 ...
 */
export function listTools(server: ToolkitDefinition): ReadonlyArray<ToolListEntry> {
  return server.tools;
}
 ...
 * - `POST *`            → run the body as a tool call, returns {@link ToolCallResult}
 *
 ...
 */
export function toFetchHandler(server: ToolkitDefinition): FetchHandler {
  return {
