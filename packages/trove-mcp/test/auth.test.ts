...
import type { McpServerConfig, McpToolCall, ToolContext } from '../src/types.js';
...
describe('defineMcpServer auth validation', () => {
  it('throws on an unknown auth type', () => {
    expect(() =>
      defineMcpServer({
        auth: { type: 'saml' } as unknown as NonNullable<McpServerConfig['auth']>,
    ...
    expect(() =>
      defineMcpServer({
        auth: {
          ...
          clientIdSecret: 'A',
          clientSecretSecret: 'B',
        } as NonNullable<McpServerConfig['auth']>,
        tools: [{ name: 't', description: 'd', input: z.object({}), handler: async () => 'x' }],
      }),
    ).toThrow(/apiHost/);
  });
